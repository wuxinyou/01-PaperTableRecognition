...
void CPTOCRDlg::MyInit()
{
	...
	{		
		...
		for (auto i = 0; i < sheetXML.vecCell.size(); i++)
		{
			//识别ROI
			string strOCR = processImage.RecROI(sheetXML.vecCell.at(i).x + processImage.outBorder.x, sheetXML.vecCell.at(i).y + processImage.outBorder.y, sheetXML.vecCell.at(i).width, sheetXML.vecCell.at(i).height);

			//因为识别后有两个'\n\n'，所有下面去掉
			strOCR.erase(strOCR.find('\n'));

			//填写excel
			excelOpera.EditExcel(sheetXML.vecCell.at(i).cellNum, strOCR);
		}
		

		
		excelOpera.ShowExcel();
		//excelOpera.ExitExcel();
	}
